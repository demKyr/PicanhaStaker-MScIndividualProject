...
from v3PicanhaMockUpQueue import queue
...
def show_state():
    ...
    print('|StakingEOA =', VaultBalances["StakingEOA"])
...
def stakingEOAInjection(amount):
    VaultBalances['StakingEOA'] += amount
    return


def stakingEOALeakage(amount):
    VaultBalances['StakingEOA'] -= amount
    return
...
def stake():
    ...
    VaultBalances["StakingEOA"] -= stakeFee
...
def unstake():
    ...
    currentSharePx = sharePx()
    VaultBalances['StakingEOA'] -= unstakeFee
    ...
    while(not Wqueue.isEmpty()):
        ...
        if(req.isDirectWithdraw):
            ...
            VaultBalances['StakingEOA'] += req.amount * withdrawFee
    return
...
def pairQueues():
    ...
    if(Wqueue.Q[Wqueue.first].isDirectWithdraw):
        ...
        VaultBalances["StakingEOA"] += amount * withdrawFee
...
def deposit(user, amount):
    ...
    VaultBalances['VaultAmount'] += amount
    VaultBalances['StakingEOA'] += amount * depositFee
    VaultBalances["DqueueTotal"] += amount 
...
def withdraw(user, amount):
    ...
    if (preshares[user]):
        ...
        VaultBalances["StakingEOA"] += tempAmount * withdrawFee
