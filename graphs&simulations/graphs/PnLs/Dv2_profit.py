...
from Dv2_importer import ApproachD
...
initialStakingEOAInjection = 1000
fee=0.1
depositFee=0.001
withdrawFee=0.001
...
def approachDsim(initialAmountInValidator, dailyDeposits, dailyWithdrawals):
    ...
    AD.initialisation(initialStakingEOAInjection = initialStakingEOAInjection)
    ...
    for day in range(num_of_days):
        ...
        StakingEOABalance.append(AD.VaultBalances['StakingEOA'] - initialStakingEOAInjection)
        profitsFromRewards.append(AD.shares['Treasury'] * AD.sharePx() + (AD.VaultBalances['claimedRewards'] + AD.VaultBalances['rewards']) * fee)
        AD.show_state()
    return profitsFromRewards, StakingEOABalance
